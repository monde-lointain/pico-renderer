...
**Change:** add the cloud sky as a SECOND blit, ADDITIVELY over the T3a panorama (zero change to the panorama path). `demo_fill_blits` now also fills `blits[1] = BLIT2D_CLOUDS` (the already-landed D.5 `blit2d_clouds`) over the visible sky band `[0, horizon_row)`: the baked I8 `g_terrain_cloud` (64×64, T3b-prep asset `50bd6e3`) composited over the N64 blue gradient. **0 module code** (no `blit2d.cc`/`rdr.cc`/`frame.h`/`types.h` touch; pure demo + test wiring). fb_crc golden rebaked 0x22856499 → **0x39b7dd60**. `main.cc` gains a `blits=%u` telemetry field (keyed parse, robust). All 20 host demo tests green; `ci_main` GREEN (host + tidy + canary + pico firmware 702 KB flash / 234 KB BSS — cloud adds 4 KB flash).
