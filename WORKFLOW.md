...
## B.1-θ platform display-fork — friction (2026-05-27)

| ID | Symptom | Root cause | Sev | → Artifact | Owner | Status |
|----|---------|-----------|-----|-----------|-------|--------|
| **W1-09** | `tests/platform/CMakeLists.txt` never registers: `ctest -L platform` = 0 tests despite 16 green | Lead-owned `tests/CMakeLists.txt` `foreach()` enumerates subsystems but omits `platform`; lane owns `tests/platform/**` only | **High (hollow gate)** | one-line contract-delta: add `platform` to the `foreach(sub … sched harness)` list in `tests/CMakeLists.txt` | Lead | **open — needs Lead** (verified green via standalone CMake build in the interim) |
| W1-10 | pico preset can't configure in worktree (`PICO SDK location not specified`) → arm-compat gate would falsely fail | `cmake --preset pico` has no `PICO_SDK_PATH`; not set in the preset env | Low | export `PICO_SDK_PATH=~/development/repos/pico-sdk` before `cmake --preset pico` (documented here; preset could carry an `environment` block) | T5/Lead | workaround applied (configured + `platform_pico` builds clean) |
| W1-11 | display-fork was already applied in the snapshot (screen.pio 16-bit, COLMOD 0x55, RGB565 pack) — risk of re-doing frozen work | template fork pre-landed; task spec described it as TODO | Low (info) | verified deltas against ST7789 §9.1.32 p224 (0x55 confirmed) rather than rewriting; net-new work = scanrace guard + reset/watchdog stub + host tests | θ | resolved |
