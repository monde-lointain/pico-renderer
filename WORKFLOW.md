...
| **W1-09** | `geom_run(cmds, Frame*)` is the frozen entry, but `struct Frame` (Lead-owned, `rdr/rdr.cc`) carries only `{fb,width,height}` — no TVtx pool, tile bins, matrix/viewport/material state, or clip scratch. geom has nowhere to emit its output, so the frozen entry cannot be implemented for real in W1. | Contract froze the *entry signature* + the cross-module POD (`TVtx`/`TileBin`/`Command`) but NOT the frame-state aggregate that wires them together; that lives behind the opaque `Frame` the Lead fills at integration (C1). | **High (blocks geom_run end-to-end; B.1-β shipped the testable core only)** | Lead contract-delta: extend `struct Frame` (or add a `GeomCtx` the entry takes) to carry `GeomOut` (pool+bins+counters) + `MtxStack`/`Viewport`/current-material + clip scratch. B.1-β exposed all of this as module-local POD (`GeomOut`, `MtxStack`, `Viewport` in `geom.h`) so C1 only has to own + pass it. | Lead | open (delta requested) |
| W1-02 | (resolved by B.1-β) geom maps `SET_VIEWPORT{x,y,w,h}` via the oracle center/extent convention; `Geom.TransformProjectMatchesOracle` + `Geom.ViewportMatchesOracle` pin it (≤1px). | — | Med | `tests/geom/geom_test.cc` cross-checks oracle | T1 | **verified** — geom project matches oracle |
