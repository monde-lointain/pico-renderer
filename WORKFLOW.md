...
| W1-09 | First `Write` to a fresh skeleton file silently did not persist (reported success; file still skeleton on re-read) → wasted a red-cycle on a stale test binary | Editing a file not yet `Read` in-session; harness only persists Write after a Read establishes file state | Low | habit: `Read` every skeleton before first `Write`/`Edit`; treat unexpected "1 test ran" as a stale-source signal and `wc -l` the source before debugging the binary | T4 (cmd-arena) | self-applied this stream |
