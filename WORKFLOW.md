...
**Change:** `raster_tile_noclear` now sweeps the bin TWICE. Sweep 1 (opaque): skip `ZMODE_XLU` refs, rasterize the rest via the UNCHANGED `raster_one` (z-test+write); `ZMODE_OPAQUE`+`ZMODE_DECAL` both here (DECAL == opaque for now; decal-dZ noted out-of-T2-scope). Sweep 2 (XLU): gather XLU bin indices + a depth key (Σ of 3 verts' `inv_w`), `qsort` ASCENDING inv_w (farthest first) with gather-order tie-break, then `raster_one_xlu` per tri — same edge/UV/Gouraud/combiner, but z-TEST only (`zbuf` param is `const` → no-z-write enforced by the type) and the store is `blend_pixel_alpha(BLEND_ALPHA, combiner_out, texel.a, fb)` (texel-alpha alpha-over, P3-3). Signatures of `raster_tile`/`_noclear` UNCHANGED → `drain.h`/dispatch untouched. Opaque path provably byte-identical (zero deleted lines in `raster_one`/`tri_setup`; goldens + demo fb_crc 0xa59eb386 unchanged).

**XLU sort-scratch decision (the re-surface trigger): fixed local `XluKey[256]` on the stack, drop-with-count — NO `frame.h` touch.** A full per-tile cap (`RDR_BIN_POOL_REFS`=2048) at 2 B/entry ≈4 KB would BLOW the RP2040 core1 stack (SDK default `PICO_CORE1_STACK_SIZE` = 2 KB; dual-core raster runs on core stacks). 256 entries = 512 B (comfortable). Overflow `++s_xlu_dropped` (surfaced via `raster_xlu_dropped()`), mirroring `TileBin.dropped` — never corrupt. 256 XLU tris in ONE 60×60 tile >> tree-foliage density, so a drop is a re-surface signal, not steady state. Chose drop-with-count over a Φ `frame.h` add to keep this lane self-contained; **if real terrain density ever trips `raster_xlu_dropped() > 0`, re-surface to Lead for a Φ per-worker XLU-scratch (sized to the per-tile cap, one per worker like zbuf/cov) rather than raising the local cap toward the stack ceiling.**

**Determinism (the load-bearing bit):** `qsort` is NOT stable, so the gather index is folded into the comparator (ascending depth, then ascending order) → reproducible order. Each tile is drawn by exactly one worker, so serial == 2-worker bit-identical holds (new `tests/sched` `SchedXluDeterminism.MixedSceneSerialEqualsTwoWorker` + teeth test `XluActuallyBlends`).
...
- **Comparator return must not subtract int64 keys.** Depth key = Σ inv_w (Q16.16) can exceed int32; the `qsort` comparator returns −1/0/+1 by `<`/`>` compares, never `a-b` (would overflow the `int` return). 
