...
TEST(Arena, AllocAlignsToEightBytes) {
  ...
  void const* p = arena_alloc(&a, 8);  // must be 8-byte aligned
...
TEST(Arena, AllocPastCapReturnsNullAndDoesNotBump) {
  ...
  uint32_t const before = a.used;
  void const* p = arena_alloc(&a, 16);  // align(24)=24, +16 = 40 > 32
  EXPECT_EQ(p, (void*)0);
...
TEST(Arena, ResetRewindsOToOneAndReusesSpace) {
  struct Arena a;
  arena_init(&a, g_buf, sizeof g_buf);
  void const* p0 = arena_alloc(&a, 64);
  arena_reset(&a);
  EXPECT_EQ(a.used, 0U);
  void const* p1 = arena_alloc(&a, 64);
