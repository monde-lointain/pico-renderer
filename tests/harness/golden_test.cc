...
TEST(Golden, FirstRunWritesThenComparePasses) {
  ...
  EXPECT_EQ(golden_check(&p, buf, W, H, &rep), GOLDEN_WROTE);

  // Second run, identical render -> PASS (exact, zero tolerance).
  EXPECT_EQ(golden_check(&p, buf, W, H, &rep), GOLDEN_PASS);
...
TEST(Golden, InjectedMismatchFails) {
  ...
  uint8_t good[W * H * 3];
  uint8_t bad[W * H * 3];
  render_scene(good, W, H, 0);
  render_scene(bad, W, H, 6);  // moved a vertex 6px -> many differing pixels
  ...
  memset(&p, 0, sizeof p);
  p.golden_path = gpath;
  p.dump_dir = ::testing::TempDir().c_str();
  p.per_channel = 8;
  p.max_diff_pixels = 0;

  struct GoldenReport rep;
  // Establish golden from the GOOD render.
  ASSERT_EQ(golden_check(&p, good, W, H, &rep), GOLDEN_WROTE);

  // Compare the BAD render -> MUST FAIL.
  int r = golden_check(&p, bad, W, H, &rep);
...
TEST(Golden, ToleranceAbsorbsSmallNoise) {
  ...
  struct GoldenReport rep;
  ASSERT_EQ(golden_check(&p, base, W, H, &rep), GOLDEN_WROTE);
  EXPECT_EQ(golden_check(&p, noisy, W, H, &rep), GOLDEN_PASS);
  ...
  p.per_channel = 2;
  EXPECT_EQ(golden_check(&p, noisy, W, H, &rep), GOLDEN_FAIL);
...
TEST(Golden, PixelBudgetAllowsBoundedDifference) {
  ...
  render_scene(base, W, H, 0);
  memcpy(noisy, base, sizeof base);
  for (int i = 0; i < 3; ++i) noisy[i * 3] = (uint8_t)(base[i * 3] ^ 0x40);
  ...
  struct GoldenReport rep;
  ASSERT_EQ(golden_check(&p, base, W, H, &rep), GOLDEN_WROTE);
  EXPECT_EQ(golden_check(&p, noisy, W, H, &rep), GOLDEN_PASS);  // 3 <= 3
  p.max_diff_pixels = 2;
  EXPECT_EQ(golden_check(&p, noisy, W, H, &rep), GOLDEN_FAIL);  // 3 > 2
  remove(gpath);
}
...
TEST(Golden, CommittedSceneRegression) {
  ...
  struct OMat4 mvp;
  for (int i = 0; i < 16; ++i) mvp.m[i] = 0.0f;
  mvp.m[0] = mvp.m[5] = mvp.m[10] = mvp.m[15] = 1.0f;  // identity
  struct OViewport vp = {0, 0, W, H};

  struct OVec4 quad[4] = {{-0.6f, -0.6f, 0.0f, 1.0f},
                          {0.6f, -0.6f, 0.0f, 1.0f},
                          {0.6f, 0.6f, 0.0f, 1.0f},
                          {-0.6f, 0.6f, 0.0f, 1.0f}};
  struct OTVtx tv[4];
  ...
  struct GoldenReport rep;
  int r = golden_check(&p, buf, W, H, &rep);
