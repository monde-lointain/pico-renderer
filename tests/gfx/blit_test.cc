...
TEST(Blit, RectDrawsHollowBorder) {
  Framebuffer fb;
  fb_clear(&fb, 0x0000);
  color_t c = rgb565(0xFF, 0xFF, 0xFF);
...
TEST(Blit, FrectFillsAllPixels) {
  Framebuffer fb;
  fb_clear(&fb, 0x0000);
  color_t c = rgb565(0x80, 0x80, 0x80);
...
static const uint16_t kCopyPixels[] = {
...
static const uint8_t kCopyMask[] = {0xFC, 0xC0};
static const Sprite kCopySprite = {kCopyPixels, kCopyMask, 3, 2};

TEST(Blit, BlitCopyWritesAllPixels) {
  Framebuffer fb;
  fb_clear(&fb, 0x0000);
  blit_copy(&fb, &kCopySprite, 10, 20, 0);         /* no flip */
...
TEST(Blit, BlitCopyHFlipReversesRows) {
  Framebuffer fb;
  fb_clear(&fb, 0x0000);
  blit_copy(&fb, &kCopySprite, 10, 20, BLIT_HFLIP);
...
TEST(Blit, BlitCopyVFlipReversesColumns) {
  Framebuffer fb;
  fb_clear(&fb, 0x0000);
  blit_copy(&fb, &kCopySprite, 10, 20, BLIT_VFLIP);
...
TEST(Blit, BlitCopyClipsRight) {
  ...
  blit_copy(&fb, &kCopySprite, 239, 20, 0);
...
TEST(Blit, BlitCopyClipsBottom) {
  Framebuffer fb;
  fb_clear(&fb, 0x5678);
  blit_copy(&fb, &kCopySprite, 10, 239,
...
TEST(Blit, BlitCopyClipsLeft) {
  Framebuffer fb;
  fb_clear(&fb, 0xAAAA);
  blit_copy(&fb, &kCopySprite, -1, 20, 0); /* col 0 off-screen */
  /* col 1 → x=0, col 2 → x=1 */
...
TEST(Blit, BlitCopyClipsTop) {
  Framebuffer fb;
  fb_clear(&fb, 0xBBBB);
  blit_copy(&fb, &kCopySprite, 10, -1, 0); /* row 0 off-screen */
  /* row 1 → y=0 */
 ...
 */
static const uint16_t kMaskPixels[] = {
    0xF800, 0x0000, 0x001F, /* red, black(ignored), blue */
    0x0000, 0x07E0, 0x0000  /* black(ignored), green, black(ignored) */
};
static const uint8_t kMaskBytes[] = {0xA8};
static const Sprite kMaskSprite = {kMaskPixels, kMaskBytes, 3, 2};

TEST(Blit, BlitMaskOnlyDrawsOpaquePels) {
  Framebuffer fb;
  color_t bg = rgb565(0x80, 0x40, 0x20);
  fb_clear(&fb, bg);
  blit_mask(&fb, &kMaskSprite, 10, 20, 0);
...
TEST(Blit, BlitMaskHFlipPreservesTransparency) {
  ...
  color_t bg = 0x1111;
  fb_clear(&fb, bg);
  blit_mask(&fb, &kMaskSprite, 10, 20, BLIT_HFLIP);
...
TEST(Blit, BlitMaskClipsCorrectly) {
  ...
  blit_mask(&fb, &kMaskSprite, -1, 20, 0);
