...
//     exception, never fb_crc-compared).
// Call AFTER rdr_begin_frame (which resets blit_count) and BEFORE
// rdr_end_frame.
...
// Fill *out with the EXACT RenderState demo_terrain_build binds for the tree
// pass: the 32x64 RGBA5551 tree0 sprite (CLAMP/BILINEAR) with COMBINE_MODULATE
// (TEXEL0 x SHADE), double-sided (CULL_NONE, N6 billboard), ZMODE_XLU +
// alpha_cmp=1 (faithful soft-edge trees = N64 G_RM_AA_ZB_XLU_SURF, combiner
// alpha = TEXEL0): bilinear-interpolating the 1-bit 5551 alpha gives an
// INTERMEDIATE alpha across the ~1-texel cutout boundary, which the XLU
// alpha-over feathers over the framebuffer -> a soft AA'd silhouette (the
// faithful-visible stand-in for the RDP coverage->VI-AA we lack; trees are a
// SINGLE XLU pass, not a 2nd draw). alpha_cmp=1 discards only the
// fully-transparent border. Pre-lit, no fog. Tests assert the bound material
// via these; the build uses the SAME fillers (no duplicated constants).
